...
const { metricsMiddleware, metricsHandler, transactionsCreatedTotal, transactionsDeletedTotal } = require('./metrics');
...
app.get('/transaction', (req, res) => {
    transactionService.getAllTransactions((err, results) => {
        if (err) {
            req.log.error({ err: err.message }, 'GET /transaction failed');
...
app.post('/transaction', (req, res) => {
    ...
    transactionService.addTransaction(amount, description, category, (err) => {
        if (err) {
            req.log.error({ err: err.message }, 'POST /transaction failed');
...
app.delete('/transaction', (req, res) => {
    transactionService.deleteAllTransactions((err) => {
        if (err) {
            req.log.error({ err: err.message }, 'DELETE /transaction (bulk) failed');
            return res.status(500).json({ message: 'could not delete transactions', error: err.message });
        }
        transactionsDeletedTotal.inc();
...
app.delete('/transaction/:id', (req, res) => {
    ...
    transactionService.deleteTransactionById(id, (err) => {
        if (err) {
            req.log.error({ err: err.message, id }, 'DELETE /transaction/:id failed');
...
app.get('/transaction/:id', (req, res) => {
    ...
    transactionService.findTransactionById(id, (err, results) => {
        if (err) {
            req.log.error({ err: err.message, id }, 'GET /transaction/:id failed');
